...
// Dashboard Summaries (Overall, Track, BetType with Hit Rate)
function renderDashboardSummaries(data) {
  ...
  const totalHitRate = totalCount > 0 ? (hitCount / totalCount * 100) : 0;
  ...
  document.getElementById('sum-total-hitrate').textContent = `${totalHitRate.toFixed(1)}% (${hitCount}/${totalCount})`;

  const balEl = document.getElementById('sum-total-balance');
  balEl.textContent = `${totalBalance >= 0 ? '+' : ''}¥${totalBalance.toLocaleString()} (的中:${hitCount} / 敗戦:${totalCount - hitCount})`;
  balEl.className = `summary-val ${totalBalance > 0 ? 'positive-bal' : ''}`;

  // Track-wise Summary Table
  const trackMap = {};
  data.forEach(r => {
    if(!trackMap[r.track]) trackMap[r.track] = { cost: 0, payout: 0, hits: 0, total: 0 };
    trackMap[r.track].cost += r.cost;
    trackMap[r.track].payout += r.payout;
    trackMap[r.track].total += 1;
    if(r.payout > 0) trackMap[r.track].hits += 1;
  });

  const trackTbody = document.getElementById('track-summary-body');
  trackTbody.innerHTML = '';
  Object.keys(trackMap).forEach(track => {
    const item = trackMap[track];
    const roi = item.cost > 0 ? (item.payout / item.cost * 100).toFixed(1) : 0;
    const hitRate = item.total > 0 ? (item.hits / item.total * 100).toFixed(1) : 0;
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td>${track}</td>
      <td>¥${item.cost.toLocaleString()}</td>
      <td>¥${item.payout.toLocaleString()}</td>
      <td>${hitRate}% (${item.hits}/${item.total})</td>
      <td class="${roi > 100 ? 'high-roi' : ''}">${roi}%</td>
    `;
    trackTbody.appendChild(tr);
  });

  // BetType-wise Summary Table
  const betMap = {};
  data.forEach(r => {
    if(!betMap[r.betType]) betMap[r.betType] = { cost: 0, payout: 0, hits: 0, total: 0 };
    betMap[r.betType].cost += r.cost;
    betMap[r.betType].payout += r.payout;
    betMap[r.betType].total += 1;
    if(r.payout > 0) betMap[r.betType].hits += 1;
  });

  const betTbody = document.getElementById('bettype-summary-body');
  betTbody.innerHTML = '';
  Object.keys(betMap).forEach(bet => {
    const item = betMap[bet];
    const roi = item.cost > 0 ? (item.payout / item.cost * 100).toFixed(1) : 0;
    const hitRate = item.total > 0 ? (item.hits / item.total * 100).toFixed(1) : 0;
    const tr = document.createElement('tr');
    tr.innerHTML = `
...
function renderAccordion(data) {
  ...
  data.forEach(r => {
    ...
    if(!cross[r.track][r.betType]) cross[r.track][r.betType] = { cost: 0, payout: 0, hits: 0, total: 0 };
    cross[r.track][r.betType].cost += r.cost;
    cross[r.track][r.betType].payout += r.payout;
    cross[r.track][r.betType].total += 1;
    if(r.payout > 0) cross[r.track][r.betType].hits += 1;
  });

  for(const track in cross) {
    let trackCost = 0, trackPayout = 0, trackHits = 0, trackTotal = 0;
    let tableRows = '';
    
    for(const bet in cross[track]) {
      const item = cross[track][bet];
      const roi = item.cost > 0 ? (item.payout / item.cost * 100).toFixed(1) : 0;
      const hitRate = item.total > 0 ? (item.hits / item.total * 100).toFixed(1) : 0;
      
      trackCost += item.cost; 
      trackPayout += item.payout;
      trackHits += item.hits;
      trackTotal += item.total;
    ...
    const totalHitRate = trackTotal > 0 ? (trackHits / trackTotal * 100).toFixed(1) : 0;
    ...
    item.innerHTML = `
      <div class="accordion-header" onclick="toggleAccordionContent(this)">
        <span>${track} (回収率: <b class="${totalRoiClass}">${totalRoi}%</b> / 的中率: <b>${totalHitRate}%</b>)</span>
        <span>▼</span>
      </div>
...
function renderHistory(data) {
  ...
  sorted.forEach(r => {
    const isHit = r.payout > 0;
    const roi = r.cost > 0 ? ((r.payout / r.cost) * 100).toFixed(1) : 0;
    const roiClass = roi > 100 ? 'high-roi' : '';
    const statusBadge = isHit 
      ? `<span class="badge-hit">的中</span>` 
      : `<span class="badge-miss">不的中</span>`;
...
function parseCSV(text) {
  ...
  for(let i=1; i<rows.length; i++) {
    ...
    const obj = {};
    headers.forEach((h, idx) => obj[h.trim()] = rows[i][i
