...
function handleInstallButtonClick() {
  if (deferredPrompt) {
    ...
    // If beforeinstallprompt is unavailable (e.g. iOS Safari, already installed, or in-app browser)
    const isIos = /iphone|ipad|ipod/.test(window.navigator.userAgent.toLowerCase());
    const guideText = document.getElementById('pwa-guide-text');
    
    if (isIos) {
      guideText.innerHTML = `
        <b>【iPhone (Safari) での追加手順】</b><br>
        1. 画面下の<b>「共有ボタン（四角から上矢印）」</b>をタップ<br>
        2. メニューを下にスクロールして<b>「ホーム画面に追加」</b>を選択<br>
        3. 右上の<b>「追加」</b>をタップすると完了です。
      `;
    } else {
      guideText.innerHTML = `
        <b>【Android / Chrome での追加手順】</b><br>
        1. ブラウザ右上の<b>「︙（メニュー）」</b>をタップ<br>
        2. <b>「アプリをインストール」</b>または<b>「ホーム画面に追加」</b>を選択してください。<br><br>
        ※すでにホーム画面に追加済みの場合はブラウザ標準仕様により追加されません。
      `;
    }
...
// Dashboard Summaries (Total, Track-wise, BetType-wise)
...
// Cross Tabulation Accordion (競馬場別 × 券種別 分析)
